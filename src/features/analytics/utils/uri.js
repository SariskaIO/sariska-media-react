...
export function parseStandardURIString(str) {
  ...
  let regex;
  let match = null; // XXX A URI string as defined by RFC 3986 does not contain any whitespace.
...
export function urlObjectToString(o) {
  ...
  if (!url.host) {
    ...
    if (domain) {
      ...
      // a pathname only.
      _fixURIStringScheme(`${APP_LINK_SCHEME}//${domain}`)); // authority
...
export function addHashParamsToURL(url, hashParamsToAdd) {
  ...
  const urlParamsArray = _objectToURLParamsArray({ ...params,
    ...hashParamsToAdd
  });
