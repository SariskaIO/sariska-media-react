 ...
 * @extends {Component}
 */
function AudioLevelIndicator(props) {
    
        const { passedAudioLevel } = props;
        ...
        for (let i = 0; i < AUDIO_LEVEL_DOTS; i++) {
            ...
            audioLevelDots.push(
                <span
                    className = { className }
                    key = { i }
                    style = {{ opacity: cappedOpacity }}>{className}</span>
            );
        
                //console.log('dots', audioLevelDots);
        return (
            <span className = 'audioindicator in-react' style={{background: 'red', color: 'yellow'}}>
                dots { audioLevelDots }
            </span>
        );
    }
