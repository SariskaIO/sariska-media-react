...
describe('E2EE Context', () => {
  ...
  describe('encode function', () => {
    ...
    it('with an audio frame', async done => {
      sendController = {
        enqueue: encodedFrame => {
          const data = new Uint8Array(encodedFrame.data); // An audio frame will have an overhead of 6 bytes with this counter and key size:
          //   4 bytes truncated signature, counter (1 byte) and 1 byte trailer.

          expect(data.byteLength).toEqual(audioBytes.length + 6); // TODO: provide test vector.
    ...
    it('with a video frame', async done => {
      sendController = {
        enqueue: encodedFrame => {
          const data = new Uint8Array(encodedFrame.data); // A video frame will have an overhead of 12 bytes with this counter and key size:
          //   10 bytes signature, counter (1 byte) and 1 byte trailer.

          expect(data.byteLength).toEqual(videoBytes.length + 12); // TODO: provide test vector.
  ...
  describe('E2EE Signature', () => {
    let privateKey;
    let publicKey; // Generated one-time using
    ...
    beforeEach(async () => {
      privateKey = await crypto.subtle.importKey('jwk', rawPrivateKey, {
        name: 'ECDSA',
        namedCurve: 'P-521'
      }, false, ['sign']);
      publicKey = await crypto.subtle.importKey('jwk', rawPublicKey, {
        name: 'ECDSA',
        namedCurve: 'P-521'
      }, false, ['verify']);
      await sender.setKey(key, 0);
    ...
    it('signs the first frame', async done => {
      sendController = {
        enqueue: encodedFrame => {
          const data = new Uint8Array(encodedFrame.data); // Check that the signature bit is set.

          expect(data[data.byteLength - 1] & 0x80).toEqual(0x80); // An audio frame will have an overhead of 6 bytes with this counter and key size:
          //   4 bytes truncated signature, counter (1 byte) and 1 byte trailer.
          // In addition to that we have the 132 bytes signature.

          expect(data.byteLength).toEqual(audioBytes.length + 6 + 132); // TODO: provide test vector for the signature.
    ...
    it('signs after ratcheting the sender key', async done => {
      ...
      await sender.encodeFunction(makeAudioFrame(), sendController); // Ratchet the key. We reimport from the raw bytes.
