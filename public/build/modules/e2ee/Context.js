...
import { isArrayEqual } from './utils'; // We use a ringbuffer of keys so we can change them and still decode packets that were
...
const ENCRYPTION_ALGORITHM = 'AES-CTR'; // https://developer.mozilla.org/en-US/docs/Web/API/AesCtrParams
...
export class Context {
  ...
  constructor(id) {
    ...
    this._currentKeyIndex = -1; // A per-sender counter that is used create the AES CTR.
  ...
  encodeFunction(encodedFrame, controller) {
    ...
    if (this._cryptoKeyRing[keyIndex]) {
      this._sendCount++; // Thіs is not encrypted and contains the VP8 payload descriptor or the Opus TOC byte.

      const frameHeader = new Uint8Array(encodedFrame.data, 0, UNENCRYPTED_BYTES[encodedFrame.type]); // Construct frame trailer. Similar to the frame header described in
      ...
      const counterView = new DataView(counter.buffer); // The counter is encoded as a variable-length field.
      ...
      frameTrailer.set(new Uint8Array(counter.buffer, counter.byteLength - counterLength)); // Since we never send a counter of 0 we send counterLength - 1 on the wire.
      ...
      return crypto.subtle.encrypt({
        ...
        const newData = new ArrayBuffer(frameHeader.byteLength + cipherText.byteLength + DIGEST_LENGTH[encodedFrame.type] + frameTrailer.byteLength);
        ...
        newUint8.set(frameTrailer, frameHeader.byteLength + cipherText.byteLength + DIGEST_LENGTH[encodedFrame.type]); // append trailer.

        return crypto.subtle.sign(AUTHENTICATIONTAG_OPTIONS, this._cryptoKeyRing[keyIndex].authenticationKey, new Uint8Array(newData)).then(async authTag => {
          const truncatedAuthTag = new Uint8Array(authTag, 0, DIGEST_LENGTH[encodedFrame.type]); // Set the truncated authentication tag.

          newUint8.set(truncatedAuthTag, UNENCRYPTED_BYTES[encodedFrame.type] + cipherText.byteLength); // Sign with the long-term signature key.

          if (signatureLength) {
            const signature = await crypto.subtle.sign(this._signatureOptions, this._signatureKey, truncatedAuthTag);
  ...
  async decodeFunction(encodedFrame, controller) {
    ...
    if (this._cryptoKeyRing[this._currentKeyIndex] && this._cryptoKeyRing[keyIndex]) {
      ...
      const signatureLength = data[encodedFrame.data.byteLength - 1] & 0x80 ? this._signatureOptions.byteLength : 0;
      const frameHeader = new Uint8Array(encodedFrame.data, 0, UNENCRYPTED_BYTES[encodedFrame.type]); // Extract the truncated authentication tag.

      const authTagOffset = encodedFrame.data.byteLength - (DIGEST_LENGTH[encodedFrame.type] + counterLength + signatureLength + 1);
      const authTag = encodedFrame.data.slice(authTagOffset, authTagOffset + DIGEST_LENGTH[encodedFrame.type]); // Verify the long-term signature of the authentication tag.

      if (signatureLength) {
        if (this._signatureKey) {
          const signature = data.subarray(data.byteLength - signatureLength - 1, data.byteLength - 1);
          const validSignature = await crypto.subtle.verify(this._signatureOptions, this._signatureKey, signature, authTag);
        ...
        } else {} // TODO: surface this to the app. We are now encrypted but can not verify.
      ...
      data.set(new Uint8Array(DIGEST_LENGTH[encodedFrame.type]), encodedFrame.data.byteLength - (DIGEST_LENGTH[encodedFrame.type] + counterLength + signatureLength + 1)); // Do truncated hash comparison of the authentication tag.
      ...
      for (let distance = 0; distance < RATCHET_WINDOW_SIZE; distance++) {
        const calculatedTag = await crypto.subtle.sign(AUTHENTICATIONTAG_OPTIONS, authenticationKey, encodedFrame.data);

        if (isArrayEqual(new Uint8Array(authTag), new Uint8Array(calculatedTag.slice(0, DIGEST_LENGTH[encodedFrame.type])))) {
          validAuthTag = true;
      ...
      const counter = new Uint8Array(16);
      counter.set(data.slice(encodedFrame.data.byteLength - (counterLength + signatureLength + 1), encodedFrame.data.byteLength - (signatureLength + 1)), 16 - counterLength);
      const counterView = new DataView(counter.buffer); // XOR the counter with the saltKey to construct the AES CTR.
      ...
      return crypto.subtle.decrypt({
        ...
        counter,
        length: CTR_LENGTH
      }, this._cryptoKeyRing[keyIndex].encryptionKey, new Uint8Array(encodedFrame.data, UNENCRYPTED_BYTES[encodedFrame.type], encodedFrame.data.byteLength - (UNENCRYPTED_BYTES[encodedFrame.type] + DIGEST_LENGTH[encodedFrame.type] + counterLength + signatureLength + 1))).then(plainText => {
        ...
        console.error(e); // TODO: notify the application about error status.
