...
export default class JingleSessionPC extends JingleSession {
  ...
  doInitialize(options) {
    ...
    if (this.isP2P) {
      ...
      pcOptions.disableSimulcast = options.disableSimulcast || options.preferH264 && !options.disableH264; // disable simulcast for screenshare and set the max bitrate to
    ...
    this.peerconnection.onnegotiationneeded = () => {
      ...
      if (browser.usesUnifiedPlan() && state === 'stable' && remoteDescription && typeof remoteDescription.sdp === 'string') {
  ...
  setVideoCodecs(preferred = null, disabled = null) {
    ...
    if (this._assertNotEnded() && preferred !== current) {
      ...
      logger.debug(`Queued setVideoCodecs task on ${this}`); // Queue and execute
  ...
  _addOrRemoveRemoteStream(isAdd, elem) {
    ...
    logger.debug(`Queued ${logPrefix} task on ${this}`); // Queue and execute
  ...
  _processRemoteRemoveSource(removeSsrcInfo) {
    const remoteSdp = browser.usesPlanB() ? new SDP(this.peerconnection.remoteDescription.sdp) : new SDP(this.peerconnection.peerconnection.remoteDescription.sdp);
  ...
  replaceTrack(oldTrack, newTrack) {
    const workFunction = finishedCallback => {
      ...
      this.peerconnection.replaceTrack(oldTrack, newTrack).then(shouldRenegotiate => {
        ...
        logger.debug(`TPC.replaceTrack finished. shouldRenegotiate = ${shouldRenegotiate}, JingleSessionState = ${this.state}, ${this}`);
        ...
        return promise.then(() => {
          if (newTrack && newTrack.isVideoTrack()) {
            logger.debug(`replaceTrack worker: setSenderVideoDegradationPreference(), ${this}`); // FIXME set all sender parameters in one go?
            // Set the degradation preference on the new video sender.

            return this.peerconnection.setSenderVideoDegradationPreference() // Apply the cached video constraints on the new video sender.
            .then(() => {
    ...
    return new Promise((resolve, reject) => {
      logger.debug(`Queued replaceTrack task. Old track = ${oldTrack}, new track = ${newTrack}, ${this}`);
      this.modificationQueue.push(workFunction, error => {
        if (error) {
  ...
  _addRemoveTrackAsMuteUnmute(isMute, track) {
    ...
    return new Promise((resolve, reject) => {
      this.modificationQueue.push(workFunction, error => {
        if (error) {
          logger.error(`_addRemoveTrackAsMuteUnmute failed. Operation - ${operationName}, peerconnection = ${this}`);
          reject(error);
        } else {
          logger.debug(`_addRemoveTrackAsMuteUnmute done. Operation - ${operationName}, peerconnection = ${this}`);
          resolve();
        }
      });
    });
  ...
  close() {
    ...
    logger.debug(`Clearing modificationQueue on ${this}...`); // Remove any pending tasks from the queue
    ...
    logger.debug(`Shutdown modificationQueue on ${this}!`); // No more tasks can go in after the close task
