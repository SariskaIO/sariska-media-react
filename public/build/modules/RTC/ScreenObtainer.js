    ...
    if (browser.isNWJS()) {
      return (_, onSuccess, onFailure) => {
        window.JitsiMeetNW.obtainDesktopStream(onSuccess, (error, constraints) => {
          ...
          // streamId to sariska-media-transport. This way the desktop
          // sharing implementation for NWJS and chrome extension
          // will be the same and sariska-media-transport will be able to
  ...
  obtainScreenOnElectron(options = {}, onSuccess, onFailure) {
    if (window.JitsiMeetScreenObtainer && window.JitsiMeetScreenObtainer.openDesktopPicker) {
      ...
      window.JitsiMeetScreenObtainer.openDesktopPicker({
        ...
        gumOptions
      }, onSuccess, onFailure), err => onFailure(new JitsiTrackError(JitsiTrackErrors.ELECTRON_DESKTOP_PICKER_ERROR, err)));
    } else {
...
function onGetStreamResponse(options = {
  ...
  if (streamId) {
    ...
    onFailure(new JitsiTrackError(JitsiTrackErrors.SCREENSHARING_GENERIC_ERROR, error));
  }
}
