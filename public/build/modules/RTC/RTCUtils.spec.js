...
import screenObtainer from './ScreenObtainer'; // TODO move webrtc mocks/polyfills into a easily accessible file when needed
...
MediaStreamTrackMock.prototype.stop = function () {
  /** intentionally blank **/
};
/**
...
describe('RTCUtils', () => {
  describe('obtainAudioAndVideoPermissions', () => {
    let getUserMediaSpy, isScreenSupportedSpy, oldMediaStream, oldMediaStreamTrack, oldWebkitMediaStream;
    beforeEach(() => {
      ...
      spyOn(screenObtainer, '_createObtainStreamMethod').and.returnValue(() => {
        /** intentional no op */
      });
      isScreenSupportedSpy = spyOn(screenObtainer, 'isSupported').and.returnValue(true);
    ...
    it('gets audio and video by default', done => {
      ...
      RTCUtils.obtainAudioAndVideoPermissions().then(streams => {
        expect(streams.length).toBe(2);
        const audioStream = streams.find(stream => stream.mediaType === 'audio');
        expect(audioStream).toBeTruthy();
        expect(audioStream.stream instanceof MediaStreamMock).toBe(true);
        expect(audioStream.stream.getAudioTracks().length).toBe(1);
        const videoStream = streams.find(stream => stream.mediaType === 'video');
        expect(videoStream).toBeTruthy();
        expect(videoStream.stream instanceof MediaStreamMock).toBe(true);
    ...
    it('can get an audio track', done => {
      ...
      RTCUtils.obtainAudioAndVideoPermissions({
        ...
        expect(streams[0].stream instanceof MediaStreamMock).toBe(true);
    ...
    describe('requesting desktop', () => {
      it('errors if desktop is not supported', done => {
        ...
        RTCUtils.obtainAudioAndVideoPermissions({
          devices: ['desktop']
        }).then(() => done.fail('obtainAudioAndVideoPermissions should not succeed')).catch(error => {
          expect(error.message).toBe('Desktop sharing is not supported!');
          done();
        });
      });
      it('can obtain a desktop stream', done => {
        spyOn(screenObtainer, 'obtainStream').and.callFake((options, success) => {
          const mediaStreamMock = new MediaStreamMock();
          mediaStreamMock.addTrack(new MediaStreamTrackMock('video', options));
          success({
