...
TraceablePeerConnection.prototype._remoteTrackAdded = function (stream, track, transceiver = null) {
  ...
  const remoteSDP = browser.usesPlanB() ? new SDP(this.remoteDescription.sdp) : new SDP(this.peerconnection.remoteDescription.sdp);
  let mediaLines;
  ...
  if (!mediaLines.length) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`No media lines for type ${mediaType} found in remote SDP for remote track: ${streamId}`)); // Abort
  ...
  if (!ssrcLines.length) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`No SSRC lines for streamId ${streamId} for remote track, media type: ${mediaType}`)); // Abort
  ...
  if (isNaN(trackSsrc) || trackSsrc < 0) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`Invalid SSRC: ${ssrcStr} for remote track, msid: ${streamId} media type: ${mediaType}`)); // Abort

    return;
  } else if (!ownerEndpointId) {
    GlobalOnErrorHandler.callErrorHandler(new Error(`No SSRC owner known for: ${trackSsrc} for remote track, msid: ${streamId} media type: ${mediaType}`)); // Abort
...
TraceablePeerConnection.prototype._createRemoteTrack = function (ownerEndpointId, stream, track, mediaType, videoType, ssrc, muted) {
  ...
  if (existingTrack && existingTrack.getTrack() === track) {
    ...
    logger.error(`${this} received a second remote track for ${ownerEndpointId} ${mediaType}, ` + 'deleting the existing track.'); // The exisiting track needs to be removed here. We can get here when Jicofo reverses the order of source-add
...
TraceablePeerConnection.prototype._mungeCodecOrder = function (description) {
  ...
  if (this.codecPreference.enable) {
    ...
    if (this.codecPreference.mimeType === CodecMimeType.VP9) {
      const bitrates = Object.values(this.videoBitrates.VP9 || this.videoBitrates); // Use only the HD bitrate for now as there is no API available yet for configuring
...
TraceablePeerConnection.prototype.addTrack = function (track, isInitiator = false) {
  ...
  this.localTracks.set(rtcId, track); // For p2p unified case, use addTransceiver API to add the tracks on the peerconnection.
...
TraceablePeerConnection.prototype.setVideoCodecs = function (preferredCodec = null, disabledCodec = null) {
  ...
  if (browser.supportsCodecPreferences()) {// TODO implement codec preference using RTCRtpTransceiver.setCodecPreferences()
...
TraceablePeerConnection.prototype.setSenderVideoDegradationPreference = function () {
  ...
  const preference = localVideoTrack.videoType === VideoType.CAMERA ? DEGRADATION_PREFERENCE_CAMERA : this.options.capScreenshareBitrate && browser.usesPlanB() // Prefer resolution for low fps share.
...
TraceablePeerConnection.prototype.setMaxBitRate = function () {
  ...
  const planBScreenSharing = browser.usesPlanB() && videoType === VideoType.DESKTOP; // Apply the maxbitrates on the video track when one of the conditions is met.
  ...
  if (!(this.options.videoQuality && this.options.videoQuality.maxBitratesVideo || planBScreenSharing && this.options.capScreenshareBitrate || browser.usesUnifiedPlan())) {
    return Promise.resolve();
  }
...
TraceablePeerConnection.prototype.setRemoteDescription = function (description) {
  this.trace('setRemoteDescription::preTransform', dumpSDP(description)); // Munge the order of the codecs based on the preferences set through config.js
...
TraceablePeerConnection.prototype.setSenderVideoConstraint = function (frameHeight = null) {
  ...
  if (this.isSimulcastOn()) {
    ...
    const encodingsEnabledState = this.tpcUtils.getLocalStreamHeightConstraints(localVideoTrack.track).map(height => height <= newHeight); // Always keep the LD stream enabled, specifically when the LD stream's resolution is higher than of the
