...
function getConstraints(um, options = {}) {
  ...
  // @see https://github.com/jitsi/sariska-media-transport/pull/136

  const isNewStyleConstraintsSupported = browser.isFirefox() || browser.isWebKitBased() || browser.isReactNative();
  ...
  if (um.indexOf('screen') >= 0) {
    if (browser.isChrome()) {
      constraints.video = {
        mandatory: getSSConstraints({ ...options,
          source: 'screen'
        }),
        optional: []
      };
  ...
  if (um.indexOf('desktop') >= 0) {
    ...
    if (browser.isElectron() && options.screenShareAudio && options.desktopStream.indexOf('screen') >= 0) {
  ...
  if (um.indexOf('video') >= 0) {
    ...
    if (browser.isWebKitBased()) {
      if (constraints.video.height && constraints.video.height.ideal) {
        constraints.video.height = {
          ideal: clonedeep(constraints.video.height.ideal)
        };
      } else {
      ...
      if (constraints.video.width && constraints.video.width.ideal) {
        constraints.video.width = {
          ideal: clonedeep(constraints.video.width.ideal)
        };
      } else {
...
class RTCUtils extends Listenable {
  ...
  init(options = {}) {
    ...
    screenObtainer.init(options, this.getUserMediaWithConstraints.bind(this));
  ...
  **/
  ...
  _newGetDesktopMedia(options) {
    ...
    return new Promise((resolve, reject) => {
      screenObtainer.obtainStream(this._parseDesktopSharingOptions(options), stream => {
        resolve(stream);
      }, error => {
        reject(error);
      });
  ...
  _getAudioAndVideoStreams(options) {
    ...
    return gumPromise.then(avStream => {
      ...
      return new Promise((resolve, reject) => {
        screenObtainer.obtainStream(this._parseDesktopSharingOptions(options), desktop => resolve({
          audioVideo,
          desktop
        }), error => {
          if (audioVideo) {
  ...
  newObtainAudioAndVideoPermissions(options) {
    ...
    const maybeRequestCaptureDevices = function () {
      ...
      const constraints = newGetConstraints(requestedCaptureDevices, otherOptions);
